...
class ExtractNCTDoFn(beam.DoFn):
  ...
  def process(self, element):
     ...
     last_name = nih_record.get('last_name')
     phone = nih_record.get('phone')
     phone_ext = nih_record.get('phone_ext')
     email = nih_record.get('email')
...
def run():
     ...
     # sql code 
     sql = 'SELECT nct_number, last_name, phone, phone_ext, email, serialid FROM nih_modeled.contacts limit 10'
     ...
     table_id = 'contacts_Beam'
     schema_id = 'nct_number:STRING, last_name:STRING, phone:STRING, phone_ext:STRING, email:STRING, serialid:INTEGER'
