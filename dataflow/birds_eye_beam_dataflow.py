...
class ExtractNCTDoFn(beam.DoFn):
  ...
  def process(self, element):
     ...
     sponsor = nih_record.get('sponsor')
     title = nih_record.get('title')
     start_year = nih_record.get('start_year')
     start_month = nih_record.get('start_month')
...
def run():
     ...
     google_cloud_options.job_name = 'birds-eye-df'
     ...
     # sql code 
     sql = 'SELECT nct_number, sponsor, title, start_year, start_month, phase, condition FROM aero_modeled.birds_eye'
     ...
     dataset_id = 'aero_modeled'
     table_id = 'birds_eye_Beam_DF'
     schema_id = 'nct_number:STRING, sponsor:STRING, title:STRING, start_year:INTEGER, start_month:INTEGER, phase:STRING, condition:STRING'
